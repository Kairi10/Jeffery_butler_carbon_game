...
//  const currentBoosters = playerName.boosters;
//  if (currentBoosters.includes('electric-car')) {
//    co2Emissions.travel = co2Emissions.travel / 2;
//  }
...
async function getName() {
  ...
  if (storedName) {
    ...
    // Add to playerScores array with the existing score or default 0
...
form.addEventListener('submit', async (event) => {
  ...
  const lastSubmissionDate = new Date(localStorage.getItem('lastSubmission'));

    // If the last submission was on the same day, prevent the form from being submitted
    ...
    localStorage.setItem('lastSubmission', currentDate.toString());
